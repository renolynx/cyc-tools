...
> 版本：2026-04-30 v2 · 基于实际仓库状态 + 飞书 schema 实测 + 用户决策反馈  
...
⚠️ **不输出**到工具层的字段（隐私）：微信号、电话、邮箱、身份证号、生日、年龄、推荐人、706链接、粉丝量

> MVP 不向前端暴露上面这些。即使 admin 后台也只在编辑场景用。

📌 **MBTI 公开**——根据用户反馈，加进 Member 对象的 `mbti` 字段并在详情页展示。
...
- RSVP 报名：**公开**，但**微信号必填 + 同微信号同活动去重**
  - 如果同微信号已报名同一活动 → 返回 `{ success: true, already_registered: true }` 不重复创建
  - 这样能挡住普通刷子，又不需要密码
...
- 自动尝试匹配成员表：**优先匹配 `称呼` 字段，回退 `姓名` 字段** → 填 `member_rec_id`，profile 链接自动生效
...
Phase B  数据层（成员 + RSVP）
   ...
   └──→ Phase D  公开成员目录 /community
             · 列表（大理 / 上海 tab 切换）
             · 详情页（profile）
             · 列表卡：头像 / 据点 / 自我介绍 / 愿分享 / 感兴趣
             · 列表卡预留「标签」位（CSS 写好但留空）
             ↓
...
Phase G  活动类型标签系统（独立大件，单独排期）
        · 新建活动类型表（远程办公/创业/健身/疗愈/...）
        · 活动表加多选字段「活动类型」
        · 老活动手工/AI 补打标
        · 计算成员 top-3 标签 + KV 缓存
        · 列表卡上的「标签」位激活
...
每个 Phase 独立部署、独立可用、独立可回滚。Phase G 单独排期，不阻塞 MVP。
...
## 12.5 Phase G — 活动类型标签系统（MVP 后单独排期）

### 12.5.1 范围说明

不在 MVP 内。MVP 上线后用户决定要不要做。

### 12.5.2 实施要点

需要做的事：
1. **新建飞书表「活动类型」**（同 base 或独立）
   - 字段：类型名 / 描述 / Icon Emoji / 配色
   - 8-12 个常用类型：远程办公 / 创业 / 健身 / 疗愈 / 艺术 / 对话 / 户外 / 学习 / ...
2. **活动表加字段「活动类型」**（多选关联或多选枚举）
3. **老活动补标**（手工 50 个 / AI 自动建议 / 让原发起者自己补）
4. **成员 top-3 标签计算**：
   - 根据 `member_rec_id` 反查 RSVP 表
   - join 活动表的「活动类型」
   - 聚合频次 → 取 top 3
   - KV 缓存 24h，参加新活动后失效
5. **激活列表卡上的「标签」位**
