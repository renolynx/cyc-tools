...
> ⚠️ **2026-05-07 v3 → v4 dayrise 升级**：daybreak-os v3 quiet 已升级为 dayrise-os v4。
>
> **v4 在 v3 基础上新增 editorial signature**（non-breaking，token additive）：
> - **Display serif**：`--cyc-font-display` (Source Serif 4 Light) → Atlas brand register hero only
> - **Mono stamp**：`--cyc-font-mono` (IBM Plex Mono) → meta / 时间 stamp / 类型 pill / admin 数字（跨层）
...
> ---
>
> ⚠️ **2026-05-02 v2 → v3 quiet 更新**：本文档 Aurora（深绿暗夜玻璃）和 Atlas（橙红渐变带）规范已**部分撤回**。
>
> **撤回原因**（commit `c8b8fcf`）：用户反馈"深绿 admin 非常丑、橙红渐变饱和度过高、字看不清"。原版触发 PRODUCT.md 三条红线 —— "讲究 > 花哨"、"有性格但不喊"、"反对被设计感"。
>
> **新策略（quiet 版，v3 保留进 v4）**：Aurora / Atlas / Daybook 三层都共享**沙底 + 三 blob** canvas，差异化通过：
...
>
> 下面 Aurora / Atlas section 的"深色玻璃"、"大理风景照 hero"等具体 token 描述**保留为长期 aspirational target**，但当前 styles.css 实现按 quiet + dayrise v4 执行。请以 styles.css 为实际权威。
...
cyc.center 同时使用 **dayrise-os 三层架构**（Atlas / Aurora / Daybook，v4 加 brand vs product register 维度），但所有 token 在大理配色基础上重新映射 —— dayrise-os 提供 *how to think* 和 *patterns*，本文档提供 *the actual values*。

## Three-Layer Mapping (v4 加 register 列)

| dayrise-os 层 | register | 在 cyc.center 的形态 | 触发 |
|---|---|---|---|
| **Atlas**（编辑/对外） | brand | 沙底 + 三 blob + 玻璃卡 + 橙色锚点 + **display serif h1** + section eyebrow | `/`、`/about`、`/events`、`/events/:id`、`/rooms`、活动详情对外 |
| **Atlas**（utility） | product | 沙底 + 玻璃卡 + 橙色锚点（不上 display serif） | `/team`、`/food`、`/event-card`、`/generator` |
| **Aurora**（工具/admin） | product | 沙底 + 顶部 2px 绿装饰线 + 紧 density + green CTA + **mono numerals** | `/admin`、`/admin/*`、`/community/admin`、设置弹窗 |
| **Daybook**（陪伴/日常） | product | **沙底 + 白玻璃浮卡 + halo** + 单色橙 temporal + 笑脸 mascot | `/me`、`/me/*`、`/community`、`/community/:id`、`/food` |
| **Daybook**（公共陪伴） | brand | halo + 大 gap（不上 display serif，主调仍温柔） | `/community/memories` |

**注意：** dayrise-os SKILL.md 里 Daybook 默认是"近白底"，但 cyc.center 用 **沙底 (`#f2ede6`)** —— 这是产品 DNA，不替换。Daybook 的 halo 阴影和 5 态笑脸保留，只是落在沙色上。

**v4 关键演化**：register（brand/product）和 layer（Atlas/Aurora/Daybook）是**两个垂直维度**。register 决定字层级密度和节奏；layer 决定色 / 装饰 / pattern。两者不互相覆盖。
...
### Environment base — 洱海三色（dayrise-os v4.1，2026-05-08）

cyc 5 色 = "苍山落日"配色，但大理是"风花雪月" — **洱海月**是蓝色的，原 5 色漏了。v4.1 补：

```css
:root {
  --cyc-erhai-deep:   #2a3f5f;   /* 洱海深 · 月光下水色 */
  --cyc-erhai:        #5a6e8a;   /* 洱海中 · 暮色水色 */
  --cyc-erhai-light:  #aab5c4;   /* 洱海浅 · 雾色水色 */
}
```

**关键**：erhai 是 **environment base**，不是 accent，不破"不引入第二种主色"原则。

| Token 角色 | 包含 | 典型用法 |
|---|---|---|
| **brand identity** | green / green-2 / orange / tan / sand | logo / 主 CTA / admin tint / pill / 装饰线 / brand 阴影 |
| **environment base**（v4.1 新加） | erhai-deep / erhai / erhai-light | hero overlay 暗化 / dusk 末端 / night 主体 / 月色场景 |
| **temporal accent** | orange | "本周" / 录制 / 今天 |

**erhai 不能下沉做 brand identity**（不能做 logo / 主 CTA / admin tint）。详见 dayrise-os SKILL.md hard rule #21。
...
:root {
  ...
  --cyc-text-xl:   28px;
  --cyc-text-2xl:  40px;
  --cyc-text-3xl:  56px;  /* hero product register only */

  /* ─── v4 dayrise additions (2026-05-07) ─── */

  /* Display serif — Atlas brand register hero only */
  --cyc-font-display: 'Source Serif 4', 'Source Han Serif SC',
                      'Songti SC', Georgia, serif;
  --cyc-font-display-weight: 300;
  --cyc-text-display-sm: 40px;
  --cyc-text-display-md: 48px;
  --cyc-text-display-lg: 56px;
  --cyc-text-display-xl: 64px;
  --cyc-tracking-display: -0.02em;
  --cyc-leading-display: 1.08;

  /* Mono stamp — IBM Plex Mono · meta / time / pill / admin nums */
  --cyc-font-mono: 'IBM Plex Mono', ui-monospace,
                   SFMono-Regular, Menlo, Monaco, Consolas, monospace;
  --cyc-text-stamp-sm: 11px;
  --cyc-text-stamp:    13px;
  --cyc-tracking-stamp: 0.04em;
  --cyc-leading-stamp:  1.4;

  /* Warm muted — Daybook long-form opt-in */
  --cyc-text-muted-warm: #5c554c;  /* 6.4:1 on sand · AAA at ≥18px or ≥14px bold */

  /* Brand register layout */
  --cyc-section-gap-brand:        96px;
  --cyc-section-gap-brand-tight:  80px;
  --cyc-content-max-brand:        1120px;
}
```
...
| Token | 范围 | 不能用于 |
|---|---|---|
| `--cyc-font-display` | Atlas brand register `<h1>` / `<h2>` 仅 hero | body, button, pill, product register, Aurora, Daybook |
| `--cyc-font-mono` | meta stamp / 时间 / 类型 pill / admin 数字（11-13px）| body, hero, CTA, 中文字符 |
| `--cyc-text-muted-warm` | Daybook long-form 中文段落 | 短 label, button text, headings |

**weight 300 仅给 display serif 用**。Inter / PingFang 仍是 400/600/700。
...
```css
.atlas-canvas {
  /* 大理风景照全屏，洱海蓝暗化叠加（v4.1 从 cyc-green 改 cyc-erhai-deep — 用户反馈墨绿丑） */
  background:
    linear-gradient(rgba(42, 63, 95, 0.45), rgba(42, 63, 95, 0.55)),
    url('<dali-scene>.jpg') center/cover fixed;
}

.atlas-card {
  background: rgba(42, 63, 95, 0.72);  /* dark erhai glass — v4.1 */
...
**Aurora 规则**：与 dayrise-os SKILL.md 一致，唯一区别是底色 hue 从 `270`（紫蓝）改为 `150`（绿）。
...
## Patterns (dayrise-os v4 在 cyc.center 落地，从 daybreak-os v2/v3 继承 + v4/v4.1 新增)
...
倾斜 ±8°，堆叠 ≤8 张 — 同 dayrise-os SKILL.md。
...
1. ❌ **不用饱和电子蓝**（如 `#3b82f6` / `#0ea5e9` 类，SaaS 标志色） —— 链接用 green，提示用 orange。**erhai 三色（带灰的洱海蓝 #2a3f5f / #5a6e8a / #aab5c4）允许**，但仅作 environment base，不下沉 brand identity（详见 dayrise-os SKILL.md hard rule #21）。
...
## 与 dayrise-os SKILL.md 的关系

- dayrise-os SKILL.md → **应该怎么思考**（layer 选择 / register 选择 / pattern 触发 / anti-pattern）
- DESIGN.md（本文档）→ **真正用什么值**（具体 hex、spacing、组件）

任何 UI 输出必须**两份都查**：先用 SKILL.md 决定 layer 和 register 和 pattern，再用 DESIGN.md 取实际 token。冲突时 DESIGN.md 优先（因为它绑定真实代码）。

任务时**强制读取顺序**：
1. PRODUCT.md（战略上下文 + register 切换原则）
2. DESIGN.md（视觉规范 + 当前 dayrise v4 实现状态）
3. styles/01-tokens.css（`:root` 的 `--cyc-*` 是真权威）
4. .claude/skills/dayrise-os/SKILL.md（patterns + 层选 + register 选）
5. cyc.center vault `03 设计/03 homepage-design ⭐` 和 `04 activity-card-system ⭐`（涉及首页 / 活动卡片时必读）
